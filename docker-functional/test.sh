    ...
    docker run -ti \
      ...
      -p 5900 \
      ${image_name} \
      /bin/bash -l -c './bin/setup.sh && git clone https://github.com/mozilla/loop.git repo && cd repo && git checkout taskcluster-experimental && mozdownload --type=daily --branch=mozilla-central && tar xfj *.bz2 && TEST_BROWSER=`pwd`/firefox/firefox TEST_SERVER=dev make functional'
